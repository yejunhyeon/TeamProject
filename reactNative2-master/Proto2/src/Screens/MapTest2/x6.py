...
def appNotify():
  ...
  counter = _counter
  data = _data
  writeUInt8(data, counter, 13) # _counter
  print(data)
...
def printLog():
  ...
  try:
    ...
    print("############### err ####################")
  print(y, p, r, end="") # index 0, 1, 2, 3 indexing
  print(" /see:", see, end="") # index 4
  print(" l_eye:", state_l, " r_eye: ", state_r, end="") # index 5, index 6
  print(" Screen x:", screen_x,"y:", screen_y, end="") # index 7, index 8
  print(" leye x,y:",IS.pupil_left_coords(), end="") # index 9, index 10, 
  print(" reye x,y:",IS.pupil_right_coords(), end="") # index 11, index 12
  print(" cnt:", _counter) # index 13
...
while cap.isOpened():
 ...
 # frame = cv2.flip(frame, 0)
  ...
  if(state_l != 0 or state_r != 0):
    fps_check = 30
    if(fps_check == 30):
      frame = frame[70:2*screen_y,120:2*screen_x]
  else: # 0
    if(fps_check > -30):
      fps_check -= 1
    if(fps_check > 0):
